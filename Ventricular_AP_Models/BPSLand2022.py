...
### The integration of the Land 2017 module was inspired by the code from: https://github.com/jtgreen/ToROrdLand/blob/main/SingleRun_ToRORdLand.py
## The BPSLand paper is https://pmc.ncbi.nlm.nih.gov/articles/PMC9198403/
def run_BPSLand_model(cycles, cycleLength, cell_type, amp=-53):
    
    model_type  = "BPSLand 2022"
    ...
    initial_conds_BPS2020 = [  -87 ,           7,              7,      145,          145 ,      # 1.  v  nai  nass  ki  kss
                ...
                0 , 0 ,0 ,1, 0 ,0]                                              # 15. XS XW Ca_TRPN TmBlocked ZETAS ZETAW
                   

    
    #Land initial conds
    # if celltype == "EPI":
    #     initial_conds_BPS2020 = initial_conds_BPS2020+[0.000125999575445634,0.000189952183128226,0.00655149435193622,0.999493972060333,0.0,0.0]
    # elif celltype=="ENDO":
    #     initial_conds_BPS2020 = initial_conds_BPS2020+[0.000156194131630688,0.000235128869057516,0.00807763107171867,0.999373435811656,0.0,0.0]
    # elif celltype == "M":
    #      initial_conds_BPS2020 = initial_conds_BPS2020+[0.000167065448972034,0.000250679417924283,0.00860262481216925,0.999331445205816,0.0,0.0]
    ...
    # constants = [cycleLength,celltype,amp,duration]
    
    tspan = (0, cycles*cycleLength)
    initial_conds = initial_conds_BPS2020
    sol = solve_ivp(fun = BPSLand2022, t_span = tspan, y0 = initial_conds, args = constants,method='LSODA',
    ...
    y_names = ["V","Nai",  "Nass",  "Ki", "Kss", "Cai",  "Cass",  "dd","Casr" , "dm", "dhf",  "dhs",  "dj" , "dhsp",  "djp", "dmL",
            ...
            "dIC1_dutta","dIC2_dutta", "dIO_dutta", "dIOb_dutta", "dO_dutta", "dOb_dutta","dS", "dW", "dCaTRPN", "dB", "dzetas", "dzetaw"]
...
@njit
def BPSLand2022(time,initial_conds,cycleLength,celltype,
            amp,duration,nao,cao,ko,maxEGTA0,undo_p,undo_CDI,undo_ICab,R,T,F,Acap,vmyo,
            vsr,vss,KmCaMK,aCaMK,bCaMK,CaMKo,KmCaM,GNa,thf_shift,btj,PKNa,
            Ahf,Ahs,bthL,GNaL,Gto,bGCaL,kCDI,tjnca,Kmn,k2n,ktaup,k1_dutta,k2_dutta,k3_dutta,
            k4_dutta,k11_dutta,k21_dutta,k31_dutta,k41_dutta,k51_dutta,k61_dutta,k52_dutta,k62_dutta,
    ...
    v,nai,  nass,  ki,  kss, cai,  cass,  val1,casr , m, hf,  hs,  j , hsp,  jp, mL,  hL,  hLp,  a,  iF, iS,  ap,  iFp,  iSp, d,Ok , Okp,val2,val3,val4,nca,val5, val6,xrf,xrs, xs1,  xs2,  xk1, Jrelnp,Jrelp,  CaMKt,  I1k , I2k,  Ck , I1kp,I2kp,  Ckp,  I1Cak,  I2Cak,  CCak, I1Cakp, I2Cakp, CCakp, jnca, EGTA, EGTAi,  RyRa,  RyRo,  RyRc,  RyRcp, C1_dutta, C2_dutta, Cb_dutta, D_dutta, IC1_dutta,IC2_dutta, IO_dutta, IOb_dutta, O_dutta, Ob_dutta,S,W,CaTRPN,B,ZETAS,ZETAW = initial_conds
    ...
    ### ################# BPSLand module
    #fraction of troponin C bound to Ca2+
    # S=max(0,X[43])
    # W=max(0,X[44])
    # CaTRPN=max(0,X[45])
    # B=X[46]
    ...
    kTRPN = 0.1#/ms same as koff
    nTRPN = 2 #same
    n_Tm = 3.0899 #2.2 #same
    beta1 = -2.4 #same
    beta0 = 2.3 #same
    Tref = 120 #same
    ca50 =0.5
    rs = 0.25 #S/(U+W+S) same
    rw = 0.5 #W/(U+W) same
    TRPN50 = 0.35
    nu=1.0002#7
    mu=2.0779#3
    ku = 1.523#/ms 1 in LandCE 2017
    kuw = 0.026 *nu #/ms same
    kws = 0.004 *mu #/ms same
    Aeff = 25 #same
    Aw = Aeff*0.25/((1-rs)*rw+rs)*(rs/0.25) # in TOR model they have *rs/0.25
    As = Aw
    gammas = 0.0085 #same 
    gammaw = 0.615
    phi = 2.23 #same
    ...
    kwu = kuw*(1/rw-1)-kws
    ksu = kws*rw*(1/rs-1)    
    # the fraction of blocked myosin binding sites on actin is denoted by B, whre U is there fraction of unblocked myosin

    #where Aw and As relate to the magnitude of the instantaneous
    # response to distortion, and cw and cs to the decay rate of distortion.
    # All variables with the subscript ‘w relate to the pre-powerstroke
    # state W, and variables with the subscript ‘s relate to the postpowerstroke state S. The variable k indicates the cell length relative
    # to the resting length
    
    gammawu = gammaw*np.abs(ZETAW)
    xb_wu_gamma = gammawu*W
    gammasu=gammas*np.maximum((ZETAS>0)*ZETAS,(ZETAS<-1)*(-ZETAS-1))
    xb_su_gamma=gammasu*S 
    
    #Length dependance of tension
    #lambda0=np.minimum(lambda_max, lambda_)
    #dydt[3]=kTRPN*(((cai*1000)/ca50)**nTRPN*(1-CaTRPN)-CaTRPN) # % untouched
    # caT50 = caT50_ref+beta1*np.minimum(0.2,lambda_-1)
    # dCaTRPN = kTRPN*((cai*1000/caT50)**nTRPN*(1-CaTRPN)-CaTRPN) 
    caT50 = ca50+beta1*np.minimum(0.2,lambda_-1)
    dCaTRPN = kTRPN*((cai*1000/caT50)**nTRPN*(1-CaTRPN)-CaTRPN) # present in both Land 2017 and BPSLand 2022
    #Lfac=np.maximum(0, 1+beta0*(lambda0+np.minimum(lambda_min,lambda0)-(1+lambda_min)))
    
    U=(1-B)-W-S # % unattached available xb = all - tm blocked - already prepowerstroke - already post-poststroke - no overlap
    dW = kuw*U-kwu*W-kws*W-xb_wu_gamma
    dS = kws*W-ksu*S-xb_su_gamma

    # XSSS=dr*0.5
    # XWSS=(1-rs)*rw*0.5
    kb = ku*(TRPN50**n_Tm)*0.5/(0.5-(rs*0.5)-(1-rs)*rw*0.5)
    #ktm_block=(ktm_unblock*(perm50**n_Tm)*0.5/(0.5-XSSS-XWSS))
    #dB=ktm_block*np.min(100,(CaTRPN**-(n_Tm/2)))*U-ktm_unblock*(CaTRPN**(n_Tm/2))*B
    dB = kb*np.minimum(CaTRPN**(-n_Tm/2),100)*U-ku*(CaTRPN**(n_Tm/2))*B

    # %velocity dependence -- assumes distortion resets on W->S
    dzetas=As*lambda_rate-cs*ZETAS # % - gamma_rate * ZETAS
    dzetaw=Aw*lambda_rate-cw*ZETAW # % - gamma_rate_w * ZETAW;
    
    # the active force
    #Ta = Lfac*Tref/rs*(S*(ZETAS+1)+ZETAW*W)#h_lambda*Tref/rs*((zetas+1)*S+zetaw*W)
    ...
    #dcai   = Bcai*(-(IpCa+ICab-2.0*INaCa_i)*Acap/(2.0*F*vmyo) -Jup*vsr/vmyo+Jleak*vsr/vmyo+Jdiff*vss/vmyo-dEGTAi) old dcai equation
    
    #dcai equation from BPSLad paper
    Bcai = 1.0/(1.0+(cmdnmax*kmcmdn/(kmcmdn+cai)**2.0))
    CaTRPNmax_conc = 0.07
    dcai = Bcai*(-(IpCa+ICab-2.0*INaCa_i)*Acap/(2.0*F*vmyo) -Jup*vsr/vmyo+Jleak*vsr/vmyo+Jdiff*vss/vmyo-dEGTAi-CaTRPNmax_conc*dCaTRPN)
...
# df, currents_df, artefact_duration = run_BPSLand_model(cycles,cycle_length,numCLs,cell_type,
#                                                    GKs_val,GKr_val,GK1_val,Gto,GNa_late_val,GNa_fast_val,GNab_val,GCab_val,GCa,GNCX_val,GpCa_val,GKb_val,GNaK_val,GClCa_val,GClb_val,amp)
